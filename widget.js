...
function attachEmotes(message) {
    let text = html_encode(message.text).trim();
    let data = message.emotes;
    
    // Удаляем возможный некорректный символ
    if (text.charCodeAt(text.length - 1) == 56320) {
        text = text.slice(0, -2).trim();
    }
    
    // Проверяем, если сообщение содержит только один эмодзи и больше ничего
    let emoteOnly = data.length === 1 && text === data[0].name;
    let emoteSize = emoteOnly ? '{singleEmoteSize}' : '{emoteSize}';
    let urlsSize = emoteOnly ? 4 : 1;
        ...
        .replace(
            ...
            function (m, key) {
                ...
                if (typeof result[0] !== "undefined") {
                    let url = result[0]['urls'][urlsSize];
                    if (provider === "twitch") {
                        return `<img class="emote" style="width: ${emoteSize}px; height: auto;" src="${url}"/>`;
                    } else {
